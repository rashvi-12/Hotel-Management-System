...
public class NewCustomer extends JFrame implements ActionListener {
	JTextField tfname,tfaddress,tfdstate,tfcity,tfemail,tfphone;
	JButton next,cancel;
	JLabel lblmeter;
	NewCustomer()
	{
		...
		lbldstate.setBounds(100,240,100,20);
		p.add(lbldstate);
		
		tfdstate=new JTextField();
		tfdstate.setBounds(240,240,200,20);
		p.add(tfdstate);
	...
	public void actionPerformed(ActionEvent ae)
	{
		if(ae.getSource()==next)
		{
			...
			String state=tfdstate.getText();
			String email=tfemail.getText();
			String phone=tfphone.getText();
			
			String query1="insert into customer values('"+name+"','"+meter+"','"+address+"','"+city+"','"+state+"','"+email+"','"+phone+"')";                               
			String query2="insert into login4 values('"+meter+"','', '"+name+"','','')";
